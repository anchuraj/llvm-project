...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createScan(
  ...
  if (ScanRedInfo->OMPFirstScanLoop) {
    llvm::Error Err =
        emitScanBasedDirectiveDeclsIR(AllocaIP, ScanVars, ScanVarsType, ScanRedInfo);
    if (Err)
      return Err;
  }
  if (!updateToLocation(Loc))
  ...
  emitBlock(ScanRedInfo->OMPScanDispatch, Builder.GetInsertBlock()->getParent());
  ...
  emitBlock(ScanRedInfo->OMPAfterScanBlock, Builder.GetInsertBlock()->getParent());
...
Error OpenMPIRBuilder::emitScanBasedDirectiveDeclsIR(
  ...
  auto BodyGenCB = [&](InsertPointTy AllocaIP,
    ...
    Builder.restoreIP(CodeGenIP);
    Value *AllocSpan = Builder.CreateAdd(ScanRedInfo->Span, Builder.getInt32(1));
    for (size_t i = 0; i < ScanVars.size(); i++) {
...
Error OpenMPIRBuilder::emitScanBasedDirectiveFinalsIR(
    SmallVector<ReductionInfo> ReductionInfos, ScanInfo *ScanRedInfo) {
  auto BodyGenCB = [&](InsertPointTy AllocaIP,
                       InsertPointTy CodeGenIP) -> Error {
    Builder.restoreIP(CodeGenIP);
    for (ReductionInfo RedInfo : ReductionInfos) {
      ...
      Value *Val =
          Builder.CreateInBoundsGEP(SrcTy, Buff, ScanRedInfo->Span, "arrayOffset");
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::emitScanReduction(
  ...
  auto BodyGenCB = [&](InsertPointTy AllocaIP,
    ...
    llvm::Value *NMin1 = Builder.CreateNUWSub(
        ScanRedInfo->Span, llvm::ConstantInt::get(ScanRedInfo->Span->getType(), 1));
...
OpenMPIRBuilder::createCanonicalScanLoops(
  ...
  Result->Span = TripCount;
  Result->OMPScanInit = splitBB(Builder, true, "scan.init");
  Builder.SetInsertPoint(Result->OMPScanInit);
  ...
  auto BodyGen = [=](InsertPointTy CodeGenIP, Value *IV) {
    ...
    Terminator->setSuccessor(0, Result->OMPScanDispatch);
    emitBlock(Result->OMPBeforeScanBlock,
              Builder.GetInsertBlock()->getParent());
    Builder.CreateBr(Result->OMPScanLoopExit);
    emitBlock(Result->OMPScanLoopExit, Builder.GetInsertBlock()->getParent());
    Builder.CreateBr(ContinueBlock);
    Builder.SetInsertPoint(Result->OMPBeforeScanBlock->getFirstInsertionPt());
  ...
  const auto &&ScanLoopGen = [&](LocationDescription Loc) -> Error {
    ...
    Builder.restoreIP((*LoopInfo)->getAfterIP());
    Result->OMPScanFinish = Builder.GetInsertBlock();
    return Error::success();
  };
  Error Err = emitScanBasedDirectiveIR(InputLoopGen, ScanLoopGen, Result);
  if (Err) {
    return Err;
  }
  return Result;
...
Expected<CanonicalLoopInfo *> OpenMPIRBuilder::createCanonicalLoop(
    const LocationDescription &Loc, LoopBodyGenCallbackTy BodyGenCB,
    Value *Start, Value *Stop, Value *Step, bool IsSigned, bool InclusiveStop,
    InsertPointTy ComputeIP, const Twine &Name, bool InScan, ScanInfo *ScanRedInfo) {
  LocationDescription ComputeLoc =
      ComputeIP.isSet() ? LocationDescription(ComputeIP, Loc.DL) : Loc;
