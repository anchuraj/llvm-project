...
static void parseTargetArgs(TargetOptions &opts, llvm::opt::ArgList &args) {
  ...
  if(llvm::Triple(opts.triple).isAMDGPU()) {
    opts.amdgpuIgnoreDenormalMode = args.hasFlag(clang::driver::options::OPT_fatomic_ignore_denormal_mode, clang::driver::options::OPT_fno_atomic_ignore_denormal_mode, false);
    opts.amdgpuFineGrainedMemory = args.hasFlag(clang::driver::options::OPT_fatomic_fine_grained_memory, clang::driver::options::OPT_fno_atomic_fine_grained_memory, false);
    opts.amdgpuRemoteMemory = args.hasFlag(clang::driver::options::OPT_fatomic_remote_memory, clang::driver::options::OPT_fno_atomic_remote_memory, false);
  }
