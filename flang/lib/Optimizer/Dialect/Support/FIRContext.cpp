...
static constexpr const char *amdgpuIgnoreDenormalModeName = "fir.amdgpu.ignore.denormal.mode";
...
bool fir::getAmdgpuIgnoreDenormalMode(mlir::ModuleOp mod) {
  if (auto attr = mod->getAttrOfType<mlir::UnitAttr>(amdgpuIgnoreDenormalModeName))
    return true;
  return false;
}

static constexpr const char *amdgpuFineGrainedMemoryName = "fir.amdgpu.fine.grained.memory";
...
bool fir::getAmdgpuFineGrainedMemory(mlir::ModuleOp mod) {
  if (auto attr = mod->getAttrOfType<mlir::UnitAttr>(amdgpuFineGrainedMemoryName))
    return true;
  return false;
}
static constexpr const char *amdgpuRemoteMemoryName = "fir.amdgpu.remote.memory";
