...
static LogicalResult EmitOMPScanDirective(mlir::omp::ScanOp &S, llvm::IRBuilderBase &builder,
 LLVM::ModuleTranslation &moduleTranslation) {
  ...
  if(ompCodeGen.OMPFirstScanLoop == IsInclusive) {
    builder.CreateCondBr(CmpI, ompCodeGen.OMPBeforeScanBlock, ompCodeGen.OMPAfterScanBlock);
  }else {
    builder.CreateCondBr(CmpI, ompCodeGen.OMPAfterScanBlock, ompCodeGen.OMPBeforeScanBlock);
  }
...
static LogicalResult
convertWorkshareLoop(Operation &opInst, llvm::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  ...
  auto bodyGen = [&](llvm::OpenMPIRBuilder::InsertPointTy ip,
    ...
    if(isInScanRegion){
      ...
      emitBlock(builder, ompCodeGen.OMPBeforeScanBlock);
    }
    //scan code
