...
//CHECK: %[[PTRTOPTRTOBUFF:.+]] = getelementptr { ptr, ptr }, ptr %0, i32 0, i32 0
//CHECK: %[[PTRTOBUFF:.+]] = load ptr, ptr %[[PTRTOPTRTOBUFF]], align 8
//CHECK: scan.init:                                        ; preds = %omp.wsloop.region
//CHECK: {{.*}}= call i32 @__kmpc_masked({{.*}}, {{.*}}, i32 0)
//CHECK: br i1 %{{.*}}, label %omp_region.body, label %omp_region.end
//CHECK: omp_region.end:                                   ; preds = %scan.init, %omp_region.body
//CHECK: call void @__kmpc_barrier({{.*}}, {{.*}})
//CHECK: omp_loop.preheader{{.*}}:                              ; preds = %omp_region.end
//CHECK: omp_loop.after:                                   ; preds = %omp_loop.exit
//CHECK: {{.*}}= call i32 @__kmpc_masked({{.*}}, {{.*}}, i32 0)
//CHECK: br i1 %{{.*}}, label %omp_region.body{{.*}}, label %omp_region.end{{.*}}
//CHECK: omp_region.end{{.*}}:                                   ; preds = %omp_loop.after, %omp.outer.log.scan.exit
//CHECK: call void @__kmpc_barrier({{.*}}, {{.*}})
//CHECK: br label %omp.scan.loop.cont
//CHECK: omp_loop.after{{.*}}:                                 ; preds = %omp_loop.exit{{.*}}
//CHECK: {{.*}}= call i32 @__kmpc_masked({{.*}}, {{.*}}, i32 0)
//CHECK: br i1 %{{.*}}, label %omp_region.body{{.*}}, label %omp_region.end{{.*}}
//CHECK: omp_region.body{{.*}}:                                ; preds = %omp_loop.after{{.*}}
//CHECK: tail call void @free(ptr {{.*}})
...
//CHECK: omp.outer.log.scan.body:                          ; preds = %omp.inner.log.scan.exit, %omp_region.body{{.*}}
//CHECK:   %[[K:.+]] = phi i32 [ 0, %omp_region.body{{.*}} ], [ %[[NEXTK:.+]], %omp.inner.log.scan.exit ]
//CHECK:   %[[I:.+]] = phi i32 [ 1, %omp_region.body{{.*}} ], [ %[[NEXTI:.+]], %omp.inner.log.scan.exit ]
...
//CHECK:   %[[BUFF:.+]] = load ptr, ptr %[[PTRTOBUFF]], align 8
...
//CHECK:   %[[BUFF2:.+]] = load ptr, ptr %[[PTRTOBUFF]], align 8
//CHECK:   %[[ARRAYOFFSET2:.+]] = getelementptr inbounds i32, ptr %[[BUFF2]], i32 %{{.*}}
...
//CHECK: omp_region.body:                                  ; preds = %scan.init
//CHECK:   %mallocsize = mul i32 101, ptrtoint (ptr getelementptr (i32, ptr null, i32 1) to i32)
//CHECK:   %arr = tail call ptr @malloc(i32 %mallocsize)
//CHECK:   store ptr %arr, ptr %[[PTRTOBUFF]], align 8
//CHECK:   call void @__kmpc_end_masked({{.*}}, {{.*}})
